...
%
 ...
 \item
  \label{lab:infinite}
 There exits $z\in \asem{\proj{\clang}{}}$ such that $z\in \alfact^{\omega}$, $\neg\ssynch[z]$
 and, for each $z'\in\asem{\proj{\clang}{}}$,
 $z\not\sqsubseteq_{\mathsf{ss}}z'$.
 \end{enumerate}
 ...
 $z'\in\asem{\proj{\clang}{}}$ such that $z\commds z' = \hat z \cat\aout\cat o$ where
 $\hat z = \ssplit[\acword]$ for a certain $\acword\in\alfint^*$ and $\aout\cat o$ is a sequence
 of unmatched 
 output action in $z$. 
 Notice that, by Lemma YYY, also $\hat z\cat\aout\cat o$ is maximal in $\asem{\proj{\clang}{}}$.
...
%
In this case, let $\aout$ be the first output action not matched in $z$. So, by Lemma ZZZ,
there is $z'\commds z$ such that $z'$ has the form
$z'=z_1\cat\aout\cat z_2$, where $z_1=\ssplit[\acword]$ for some $\acword\in\alfint^*$.
By definition of $\asem{\proj{\clang}{}}$, 
for each $\px\in\ptpof[z_1\cat\aout\cat z_2]$  there is a $z_{\px}\in\proj{\clang}{X}$ such that
$\filter{(z_1\cat\aout\cat z_2)}{\px}=z_{\px}$. 
In particular, using Lemma TTT, $z_{\p}= \filter{z_1}{\p}\cat\aout$ and,
for  $\px\neq\p$, $z_{\px} = \filter{z_1}{\px}\cat  \filter{z_2}{\px}$.
Let now $\hat \acword$ the element of $\clang$ such that $\proj{\hat \acword}{A} = z_{\p}$.
Since  $z_{\p}= \filter{z_1}{\p}\cat\aout\cat\filter{z_2}{\p}$, we have that
$\hat \acword$ has the form $\hat \acword = \acword_1\cat\gint\cat\acword_2$.
Now, by setting $\acword=\join[{z_1}]$, by the above
we have that
\begin{itemize}
\item
$\proj{\acword_1}{A} = \proj{\acword}{A}$\qquad 
(since $\proj{\acword_1}{A}=\filter{z_1}{\p}=\proj{\join[z_1]}{A}=\proj{\acword}{A}$)
\item
$\acword_1\cat\gint\in\clang$
\end{itemize} 
So, by $\cuuiasym[\clang]$, $\acword\cat\gint=\join[z_1]\cat\gint\in\clang$. 
We notice now that 
$\proj{(\acword\cat\gint)}{B} = 
  \proj{(\join[z_1]\cat\gint)}{B} =
  \filter{(z_1\cat\ain)}{\q}$.
Moreover, since $\aout$ is unmatched in $\aout\cat z_2$,
we get that  $\filter{(z_1\cat\ain)}{\q} = \filter{(z_1\cat\aout\cat\ain\cat z_2)}{\q}$.
So, from what previously inferred and the above, we can obtain
\begin{itemize}
\item
$ \filter{(z_1\cat\aout\cat \ain\cat z_2)}{\p}= \filter{(z_1\cat\aout)}{\p} = z_{\p}\in\proj{\clang}{A}$
\item
$ \filter{(z_1\cat\aout\cat\ain\cat z_2)}{\q} = \filter{(z_1\cat \ain)}{\q} = \proj{(\acword\cat\gint)}{B}\in\proj{\clang}{B}$
\item
$ \filter{(z_1\cat\aout\cat\ain\cat z_2)}{\px} =  \filter{(z_1\cat z_2)}{\px}  = z_{\px}\in\proj{\clang}{X}
\qquad \text{for } \px\not\in\Set{\p,\q}$
\end{itemize}
So, by definition of $\asem{\proj{\clang}{}}$, we get $z_1\cat\aout\cat\ain\cat z_2\in\asem{\proj{\clang}{}}$.
Since the number of unmatched output in $z$ is finite, by iterating the above procedure
we get an action of word $\tilde z$ such that $z\sqsubseteq\tilde z$, contradictingour assumption.
\qed
 \end{description}
\end{proof} 
% \begin{equation}
