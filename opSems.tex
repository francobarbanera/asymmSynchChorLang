...
\begin{enumerate}[i)]
\item
Given $z\in \asem{\aCS}$, there exists $z'\in \asem{\aCS}$ such that $z'\commds z$ and $\psynch[z']$.
\item
$\asem{\aCS}$ is $\commds$-closed.
\end{enumerate}
\end{lemma}
...
[say that the  semantics is  a g-language, that is it is prefix-closed]
...
\begin{example}
\label{ex:simple2}
The two synchronous semantics are trivially non equivalent in general on communicating systems.
...
A c-language $\clang$ on $\ptpset$ is CUIasym $(\cuuiasym[\clang])$ whenever,
 ...
 \begin{enumerate}[a)]
 \item (no sender-receiver cycles) For each $n$: $\acword\cat\gint[][B_i][m_i][B_{i+1}]\in\clang$ ($0\leq i\leq n$)
  implies $\ptp[B_0]\neq\ptp[B_{i+1}]$;
% and $\acword\cat\gint[][C][x][A]$. In general, a cycle. [Generalise for a cycle of arbitrary length].
  \item
  \label{def:cuiasym-I}
$\proj{\acword}{A} = \proj{\acword_1}{A}\ \land\  \acword_1\cat\gint\in\clang\ \implies\ \acword\cat\gint\in\clang$.
%  \item[ALT b]
%\label{def:cuiasym-I}
...
$$
\cuuiasym[\clang] \quad \implies \quad \sind[\proj{\clang}{}]
...
%
 ...
 In such a case, by definition, the following holds.
\begin{quote}
...
%===========================
This, by distinguishing the finite and infinite case for the maximal $z$, is equivalent to the disjunction
of the two following conditions.
 ...
 \item[ \ref{lab:finite})]
 %
 By Lemma XXX, since $\neg\ssynch[z]$ and $\wsynch[z]$ there exists 
 $z'\in\asem{\proj{\clang}{}}$ such that 
 $$z\commds z' = \hat z \cat\aout[B_0][B_1][][m_1]\cat\aout[B_1][B_2][][m_2]\ldots\cat\aout[B_{n-1}][B_n][][m_n]\cat o$$
  where
  \begin{itemize}
